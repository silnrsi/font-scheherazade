...
#     1) Without `-c`, it writes a complete and valid classes xml file to the output. The file can
#        be used as-is or segments cut and pasted into a project file. In this case there are extra
#        comments added to the file.
#     2) With `-c` it reads the named classes xml file and any classes named therein that are
#        computed by the program are replaced. In this case the only comments are the "autogenerated"
#        comment associated with each replaced class.
#
# In code comments, we'll refer to these as Mode1 and Mode2.
#
...
# If -c is not supplied, the tool uses joins the all text segments together along, along with the enclosing
# `classes` element and writes them to the output. If -c is supplied, the tool parses the xml file and then
# uses etree.fromstring() to create sub-elements to replace ones in the parsed file.
# --------------------
...
def doit(args):
    ...
    if args.classfile:
        ...
        with open(args.classfile,"r",) as f:
            tree = etree.parse(f)
    ...
    if classes is not None:
        logger.log(f'{nClasses} classes updated in "{args.output.name}".', 'P')
    else:
        logger.log(f'{nClasses} classes written to "{args.output.name}". Not all that were output are necessarily needed ' +
